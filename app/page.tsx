...
function OwnAColorContent() {
  ...
  const [tempClaim, setTempClaim] = useState<string | null>(null)
        ...
        <div className="custom-scrollbar" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '16px', padding: '20px', maxHeight: '600px', overflowY: 'auto', backgroundColor: 'rgba(0,0,0,0.2)', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {tempClaim && (
            <div className="animate-pulse" style={{ backgroundColor: 'rgba(59, 130, 246, 0.2)', border: '2px solid #3b82f6', borderRadius: '16px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ height: '100px', backgroundColor: `#${tempClaim.replace('#','')}` }} />
              <div style={{ padding: '12px', textAlign: 'center' }}>
                <p style={{ fontWeight: 'bold', fontFamily: 'monospace' }}>#{tempClaim.replace('#','')}</p>
...
export default function OwnAColor() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', backgroundColor: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Loader2 className="animate-spin" style={{ color: '#3b82f6' }} size={48} /></div>}>
      <OwnAColorContent />
    </Suspense>
  )
}
